...
DEFAULT_GENERAL_NAME = "Sheffield"
DEFAULT_SCHOOL_NAME = "The Sheffield UTC Academy Trust"
...
def main() -> None:
    ...
    source_value = args.source or config.get("source_url", SOURCE_URL)
    ...
    general_name = DEFAULT_GENERAL_NAME
    school_name = args.school_name or config.get("school_name", DEFAULT_SCHOOL_NAME)
    ...
    bank_source = args.bank_holidays_source or config.get(
        "bank_holidays_source", BANK_HOLIDAY_URL
    )
    ...
    if isinstance(leas_config, list) and leas_config:
        ...
        for lea in leas_config:
            if not isinstance(lea, dict):
                continue
            lea_name = str(lea.get("name", DEFAULT_GENERAL_NAME)).strip() or DEFAULT_GENERAL_NAME
        ...
        for lea_name, valid_years in lea_years_map.items():
            ...
            lea_json = build_school_json(valid_years, lea_name, [])
        ...
        if isinstance(schools_config, list) and schools_config:
            ...
            for school in schools_config:
                if not isinstance(school, dict):
                    continue
                name = str(school.get("name", DEFAULT_SCHOOL_NAME)).strip() or DEFAULT_SCHOOL_NAME
                lea_name = str(school.get("lea", DEFAULT_GENERAL_NAME)).strip() or DEFAULT_GENERAL_NAME
                ...
                school_json = build_school_json(valid_years, name, pd_days)
    ...
    term_json = build_term_json(valid_years, general_name, [])
    school_json = build_school_json(valid_years, school_name, pd_days)
